...
st.set_page_config(
    page_title="Cockpit Trader Pro Live - Cloud Persistent",
    page_icon="⚡",
    layout="wide",
...
# ==========================================================
# ☁️ CONNECTEUR CLOUD UPSTASH REDIS (PERSISTANCE PERMANENTE)
# ==========================================================
...
# ==========================================================
# 🪙 GESTION HYBRIDE DES PAIRES (CLOUD + FALLBACK LOCAL)
# ==========================================================
def charger_paires_radar():
    ...
    # 3. Valeurs par défaut immuables
    paires_defaut = ["SOL", "BTC", "ETH", "XRP", "ZEC", "PIPPIN", "BNB", "PIXEL"]
...
def get_mexc_futures_url(paire_str):
    base = paire_str.replace("/USDT", "").replace("-USD", "").replace("USDT", "").strip().upper()
...
def obtenir_prix_live_multi_sources(bases_actives):
    ...
    try:
        ...
        res = requests.get(url_mexc, headers={"User-Agent": "Mozilla/5.0"}, timeout=1.5).json()
        for it in res:
...
def fetch_single_mexc_kline(base, interval, limit=60):
    try:
        ...
        if not res or not isinstance(res, list):
            return base, None
        df = pd.DataFrame(res, columns=["time", "Open", "High", "Low", "Close", "Volume", "c_time", "qav"])
...
def charger_donnees_marche_globales(bases_actives):
    ...
    with ThreadPoolExecutor(max_workers=10) as executor:
        ...
        for interval in ["15m", "5m", "1m"]:
            for base in bases_actives:
                futures.append((interval, executor.submit(fetch_single_mexc_kline, base, interval, 60)))
...
def obtenir_stats_mtf_radar(nom_court):
    stats = {
        "range_15m": "N/A", "rsi_15m": "50.0",
        "range_30m": "N/A", "rsi_30m": "50.0",
        "range_1h": "N/A", "rsi_1h": "50.0",
        "range_4h": "N/A", "rsi_4h": "50.0",
        "range_1d": "N/A", "rsi_1d": "50.0"
    }
    try:
        ...
        if df_15 is not None and len(df_15) >= 20:
            ...
            df_30 = df_15.resample("30min").agg({"High": "max", "Low": "min", "Close": "last"}).dropna()
            if len(df_30) >= 14:
                ...
                stats["range_30m"] = f"[{formater_prix(low_30)} - {formater_prix(high_30)}]"
                stats["rsi_30m"] = f"{rsi_30:.1f}"

            df_1h = df_15.resample("1h").agg({"High": "max", "Low": "min", "Close": "last"}).dropna()
            if len(df_1h) >= 14:
                ...
                stats["range_1h"] = f"[{formater_prix(low_1h)} - {formater_prix(high_1h)}]"
                stats["rsi_1h"] = f"{rsi_1h:.1f}"

            df_4h = df_15.resample("4h").agg({"High": "max", "Low": "min", "Close": "last"}).dropna()
            if len(df_4h) >= 5:
                rsi_4h = float(calculer_rsi_series(df_4h["Close"], min(len(df_4h)-1, 14)).iloc[-1])
                high_4h = float(df_4h["High"].iloc[-12:].max())
                low_4h = float(df_4h["Low"].iloc[-12:].min())
                stats["range_4h"] = f"[{formater_prix(low_4h)} - {formater_prix(high_4h)}]"
        ...
        if df_1d is not None and len(df_1d) >= 14:
            ...
            stats["range_1d"] = f"[{formater_prix(low_1d)} - {formater_prix(high_1d)}]"
...
# ==========================================================
# 👥 GESTION HYBRIDE DES COMPTES (CLOUD + FALLBACK LOCAL)
# ==========================================================
def charger_tous_les_comptes():
    comptes_defaut = {
        "Thomas": {"solde": 1000.0, "capital_initial": 1000.0, "auto_actif": False, "master_auto": False, "positions": {}, "historique": []},
        "Alex": {"solde": 1000.0, "capital_initial": 1000.0, "auto_actif": False, "master_auto": False, "positions": {}, "historique": []},
        "Yeepse": {"solde": 1000.0, "capital_initial": 1000.0, "auto_actif": False, "master_auto": False, "positions": {}, "historique": []}
    }

    # 1. Tentative Cloud
    c_cloud = cloud_get("comptes_traders")
    if c_cloud and isinstance(c_cloud, dict):
...
def mettre_a_jour_un_compte(nom_trader, modificateur_fn):
    ...
    if nom_trader not in comptes:
        comptes[nom_trader] = {
            "solde": 1000.0, "capital_initial": 1000.0, "auto_actif": False,
            "master_auto": False, "positions": {}, "historique": []
        }
...
def charger_experience_ia_collective(bases_actives):
    ...
    return {
        ...
        "cooldowns": {"SMC": 0, "Momentum": 0, "Tendance": 0, "Squeeze Breakout": 0},
        "pertes_consecutives": {"SMC": 0, "Momentum": 0, "Tendance": 0, "Squeeze Breakout": 0},
        "lecons_apprises": [
            "ADN 1M XP Actif : Squeeze 15m + Grille 0.35% Maker (Profit Factor 3.08).",
            "Mémoire Cloud Permanente branchée (Upstash Redis).",
        ],
    }
...
def mettre_a_jour_ia_collective(nom_trader, paire_brute, motif_famille, win, pnl):
    ...
    paire = paire_brute.split("/")[0].split("-")[0].replace("USDT", "").strip().upper()
    ...
    if win:
        ...
        ia["pertes_consecutives"][motif_famille] = ia["pertes_consecutives"].get(motif_famille, 0) + 1
...
def charger_news_statiques():
    try:
        flux = feedparser.parse("https://news.google.com/rss/search?q=crypto+bitcoin+solana+when:2d&hl=en-US&gl=US&ceid=US:en")
        items = []
        for entry in flux.entries[:3]:
            titre = entry.title
            compound = analyzer.polarity_scores(titre)["compound"]
            tag = '<span class="tag-bull">BULLISH</span>' if compound >= 0.15 else ('<span class="tag-bear">BEARISH</span>' if compound <= -0.15 else '<span class="tag-neu">NEUTRE</span>')
...
def analyser_crypto_master_live(symbol_base):
    try:
        ...
        val = df["Close"] - ((df["High"].rolling(20).max() + df["Low"].rolling(20).min()) / 2.0 + df["SMA20"]) / 2.0
        ...
        if not sq_on and abs(mom) > 0.03:
            if mom > 0 and p > e50:
                breakout_signal = {
                    "sens": "LONG 🟢", "entree": p,
                    "sl": round(p - dist, 6), "tp1": round(p + (2.2 * dist), 6), "tp2": round(p + (4.5 * dist), 6),
                    "dist": dist, "levier": 25, "marge": 100.0
                }
        ...
        return {
            "sym": symbol_base, "prix": p, "atr": atr, "squeeze_on": sq_on,
            "momentum": mom, "ema50": e50, "grid_levels": grid_levels,
            "grid_step": GRID_STEP * 100, "breakout_signal": breakout_signal
        }
    except Exception:
        return None
...
def detecter_setup_a_plus_du_jour(donnees_globales, bases_actives):
    ...
    for nom_court in bases_actives:
        try:
            ...
            atr_15 = float(pd.concat([hl, hc, lc], axis=1).max(axis=1).rolling(14).mean().iloc[-1])

            sweep_h = any(df_15["High"].iloc[-k] > high_s and df_15["Close"].iloc[-k] < high_s for k in range(1, 3))
            sweep_l = any(df_15["Low"].iloc[-k] < low_s and df_15["Close"].iloc[-k] > low_s for k in range(1, 3))
            fvg_bear = (df_15["Low"].iloc[-3] > high_15) and ((df_15["Low"].iloc[-3] - high_15) > (atr_15 * 0.15))
            fvg_bull = (low_15 > df_15["High"].iloc[-3]) and ((low_15 - df_15["High"].iloc[-3]) > (atr_15 * 0.15))

            if (sweep_h or fvg_bear) and (prix < ema_50):
                entree_opt = prix
                dist = max(high_15 - entree_opt + (0.15 * atr_15), 0.50 * atr_15, prix * 0.008)
                sl = entree_opt + dist
                tp = entree_opt - (4.2 * dist)
                setups_valides.append({
                    "paire": f"{nom_court}/USDT", "sens": "SHORT 🔴", "entree": entree_opt,
                    "sl": sl, "tp": tp, "levier": 50, "marge_suggeree": 50.0,
                    "gain_vise": round((4.2 * dist / entree_opt) * (50.0 * 50), 2),
                    "perte_max": round((dist / entree_opt) * (50.0 * 50), 2),
                    "motif": "Sweep 15m + Rejet Pullback FVG",
                })
            elif (sweep_l or fvg_bull) and (prix > ema_50):
                entree_opt = prix
                dist = max(entree_opt - low_15 + (0.15 * atr_15), 0.50 * atr_15, prix * 0.008)
                sl = entree_opt - dist
                tp = entree_opt + (4.2 * dist)
                setups_valides.append({
                    "paire": f"{nom_court}/USDT", "sens": "LONG 🟢", "entree": entree_opt,
                    "sl": sl, "tp": tp, "levier": 50, "marge_suggeree": 50.0,
                    "gain_vise": round((4.2 * dist / entree_opt) * (50.0 * 50), 2),
                    "perte_max": round((dist / entree_opt) * (50.0 * 50), 2),
...
def analyser_profil(profil_court, donnees_globales, bases_actives):
    ...
    for nom_court in bases_actives:
        try:
            ...
            sweep_15_h = any(df_15["High"].iloc[-k] > high_s_15 and df_15["Close"].iloc[-k] < high_s_15 for k in range(1, 3))
            sweep_15_l = any(df_15["Low"].iloc[-k] < low_s_15 and df_15["Close"].iloc[-k] > low_s_15 for k in range(1, 3))
            ...
            atr_1m = float(pd.concat([hl, hc, lc], axis=1).max(axis=1).rolling(14).mean().iloc[-1])

            vol_fort = df_1["Volume"].iloc[-1] > (1.35 * df_1["Volume"].rolling(15).mean().iloc[-1])
            gros_corps = abs(prix - open_p) > (1.0 * atr_1m)

            fvg_bear_1m = (df_1["Low"].iloc[-3] > high) and ((df_1["Low"].iloc[-3] - high) > (atr_1m * 0.15))
            fvg_bull_1m = (low > df_1["High"].iloc[-3]) and ((low - df_1["High"].iloc[-3]) > (atr_1m * 0.15))

            mss_baissier = (prix < low_s_1m) and (prix < open_p)
            mss_haussier = (prix > high_s_1m) and (prix > open_p)

            signal, opt_p, sl, tp1, tp2, motif, motif_famille = None, None, None, None, None, "", "SMC"

            if profil_court == "Conservateur":
                motif_famille = "Tendance"
                if prix < ema_50_15 and sweep_15_h and mss_baissier and gros_corps:
                    signal, motif = "🔴 SHORT", "MSS 15m + Sweep Majeur"
                    opt_p = prix
                    dist = max(high_s_1m - opt_p + (0.15 * atr_1m), 0.70 * atr_1m, prix * 0.008)
                    sl = opt_p + dist
                    tp1 = opt_p - (1.8 * dist)
                    tp2 = opt_p - (3.5 * dist)
                elif prix > ema_50_15 and sweep_15_l and mss_haussier and gros_corps:
                    signal, motif = "🟢 LONG", "MSS 15m + Sweep Majeur"
                    opt_p = prix
                    dist = max(opt_p - low_s_1m + (0.15 * atr_1m), 0.70 * atr_1m, prix * 0.008)
                    sl = opt_p - dist
                    tp1 = opt_p + (1.8 * dist)
                    tp2 = opt_p + (3.5 * dist)
                ...
                if en_killzone and (sweep_15_h or sweep_15_l):
                    if sweep_15_h and mss_baissier and (fvg_bear_1m or vol_fort):
                        signal, motif = "🔴 SHORT", "Killzone MSS + FVG"
                        opt_p = prix
                        dist = max(high_s_1m - opt_p + (0.12 * atr_1m), 0.60 * atr_1m, prix * 0.0065)
                        sl = opt_p + dist
                        tp1 = opt_p - (1.8 * dist)
                        tp2 = opt_p - (3.2 * dist)
                    elif sweep_15_l and mss_haussier and (fvg_bull_1m or vol_fort):
                        signal, motif = "🟢 LONG", "Killzone MSS + FVG"
                        opt_p = prix
                        dist = max(opt_p - low_s_1m + (0.12 * atr_1m), 0.60 * atr_1m, prix * 0.0065)
                        sl = opt_p - dist
                        tp1 = opt_p + (1.8 * dist)
                        tp2 = opt_p + (3.2 * dist)
                ...
                if vol_fort:
                    if (prix < ema_50_15) and mss_baissier and (fvg_bear_1m or gros_corps):
                        signal, motif = "🔴 SHORT", "MSS 1m + Momentum"
                        opt_p = prix
                        dist = max(high_s_1m - opt_p + (0.10 * atr_1m), 0.55 * atr_1m, prix * 0.0055)
                        sl = opt_p + dist
                        tp1 = opt_p - (1.8 * dist)
                        tp2 = opt_p - (3.5 * dist)
                    elif (prix > ema_50_15) and mss_haussier and (fvg_bull_1m or gros_corps):
                        signal, motif = "🟢 LONG", "MSS 1m + Momentum"
                        opt_p = prix
                        dist = max(opt_p - low_s_1m + (0.10 * atr_1m), 0.55 * atr_1m, prix * 0.0055)
                        sl = opt_p - dist
                        tp1 = opt_p + (1.8 * dist)
                        tp2 = opt_p + (3.5 * dist)
                ...
                motif_famille = "SMC"
                if (sweep_15_h or prix < ema_50_15) and mss_baissier and fvg_bear_1m:
                    signal, motif = "🔴 SHORT", "Ancrage 15m ➔ FVG 1m"
                    opt_p = prix
                    dist = max(high_s_1m - opt_p + (0.10 * atr_1m), 0.50 * atr_1m, prix * 0.0050)
                    sl = opt_p + dist
                    tp1 = opt_p - (1.8 * dist)
                    tp2 = opt_p - (3.8 * dist)
                elif (sweep_15_l or prix > ema_50_15) and mss_haussier and fvg_bull_1m:
                    signal, motif = "🟢 LONG", "Ancrage 15m ➔ FVG 1m"
                    opt_p = prix
                    dist = max(opt_p - low_s_1m + (0.10 * atr_1m), 0.50 * atr_1m, prix * 0.0050)
                    sl = opt_p - dist
                    tp1 = opt_p + (1.8 * dist)
                    tp2 = opt_p + (3.8 * dist)

            en_cooldown = ts_actuel < ia_data.get("cooldowns", {}).get(motif_famille, 0)
            if signal and en_cooldown:
                signal = None

            nom_paire = f"{nom_court}/USDT"
            resultats.append({
                "Paire": nom_paire, "Nom_Court": nom_court, "Prix": prix, "High": high, "Low": low,
                "Signal_Detecte": signal, "Motif": motif, "Motif_Famille": motif_famille,
                "Opt_Price": opt_p, "SL": sl, "TP1": tp1, "TP2": tp2, "RSI": f"{rsi_val:.1f}", "Intervalle": "1m"
            })
        except Exception:
            continue
    return resultats
...
choix_langue = st.sidebar.selectbox(
    ...
    index=0 if st.session_state.langue == "FR" else 1, label_visibility="collapsed"
)
...
if "trader_session" not in st.session_state:
    st.session_state.trader_session = "Thomas" if "Thomas" in liste_noms else liste_noms[0]

st.sidebar.markdown(f"### 👤 {t('user_space')}")
idx_nom = liste_noms.index(st.session_state.trader_session) if st.session_state.trader_session in liste_noms else 0
choix_trader = st.sidebar.selectbox(t("connected_as"), liste_noms, index=idx_nom, key="select_trader_box")
...
if up_url:
    st.sidebar.markdown("<span style='color:#14F195; font-size:12px; font-weight:bold;'>☁️ Mémoire Cloud Active (Upstash)</span>", unsafe_allow_html=True)
else:
    st.sidebar.markdown("<span style='color:#FFB300; font-size:11px;'>⚠️ Stockage Local (Configure les Secrets Upstash)</span>", unsafe_allow_html=True)

# GESTIONNAIRE DYNAMIQUE DE PAIRES
with st.sidebar.expander(f"🪙 {t('manage_pairs')}", expanded=False):
    st.caption(t("manage_pairs_sub"))
    nouvelle_paire_in = st.text_input(t("add_crypto_ph"), key="input_new_crypto").strip().upper()
    if st.button(t("add_crypto_btn"), key="btn_add_crypto"):
        if nouvelle_paire_in:
            nom_nettoye = nouvelle_paire_in.replace("/USDT", "").replace("-USD", "").replace("USDT", "").strip().upper()
...
compte_actif = comptes_actuels.get(trader_courant, {"solde": 1000.0, "capital_initial": 1000.0, "auto_actif": False, "master_auto": False, "positions": {}, "historique": []})
...
with col_h1:
    st.markdown(f"### ⚡ {t('cockpit_title')} <span class='user-badge'>👤 {trader_courant}</span>", unsafe_allow_html=True)
with col_h2:
...
profil = st.select_slider(
    t("active_profile"),
    options=["🛡️ Conservateur (15m x20)", "⚖️ Intraday (5m x50)", "⚡ Scalping (1m x100)", "🔥 Ultra-Scalp (1m x150)"],
    value="🔥 Ultra-Scalp (1m x150)"
)

if "Conservateur" in profil:
    profil_cle, levier_suggere, duree_memoire, unite_temps = "Conservateur", 20, 600, "15m"
elif "Intraday" in profil:
    profil_cle, levier_suggere, duree_memoire, unite_temps = "Intraday", 50, 300, "5m"
elif "Scalping" in profil:
    profil_cle, levier_suggere, duree_memoire, unite_temps = "Scalping 1m", 100, 180, "1m"
else:
    profil_cle, levier_suggere, duree_memoire, unite_temps = "Ultra-Scalp", 150, 120, "1m"

durees_profils = {"Conservateur": 600, "Intraday": 300, "Scalping 1m": 180, "Ultra-Scalp": 120}
...
def bloc_live_auto_actualise():
    ...
    # 1. Setup A+ Royal
    setup_a_plus = detecter_setup_a_plus_du_jour(donnees_globales, bases_actives)
    if setup_a_plus:
        ...
        st.markdown(f"""<div class="gold-card-empty">{t('setup_a_waiting')}</div>""", unsafe_allow_html=True)

    # 2. Synchronisation profils
    ...
    for p_nom in LISTE_PROFILS:
        ...
        a_suppr = [p for p, info in mem_p.items() if maintenant_ts - info["timestamp"] > exp]
        for p in a_suppr:
            del mem_p[p]
    ...
    with col_c:
        st.markdown(f"""<div class="mini-card-conservateur"><b>🛡️ Conservateur (15m x20)</b><br>""", unsafe_allow_html=True)
        sigs_c = st.session_state.memoire_par_profil.get("Conservateur", {})
        if sigs_c:
            for p, info in sigs_c.items():
                st.markdown(f"**{info['signal']}** `{p}` 🎯 {formater_prix(info['prix_entree'])}", unsafe_allow_html=True)
        else:
            st.markdown(f"<span style='color:#6E7681;'>{t('veille')}</span>", unsafe_allow_html=True)
        st.markdown("</div>", unsafe_allow_html=True)

    with col_i:
        st.markdown(f"""<div class="mini-card-intraday"><b>⚖️ Intraday (5m x50)</b><br>""", unsafe_allow_html=True)
    ...
    with col_s:
        st.markdown(f"""<div class="mini-card-scalping"><b>⚡ Scalp (1m x100)</b><br>""", unsafe_allow_html=True)
    ...
    with col_u:
        st.markdown(f"""<div class="mini-card-ultrascalp"><b>🔥 Ultra (1m x150)</b><br>""", unsafe_allow_html=True)
    ...
    # 4. MOTEUR AUTO-TRADER UNIVERSEL
    ...
    def executer_moteur_complet(compte):
        ...
        # A. Gestion des positions Master
        for pos_k in list(compte.get("positions", {}).keys()):
            if "Master_" in pos_k:
                ...
                pnl_flottant_m = (((p_direct - entree_m) / entree_m) * notionnel_m if is_long_m else ((entree_m - p_direct) / entree_m) * notionnel_m)
                roe_m = (pnl_flottant_m / marge_m) * 100

                # Profit-locking
                if roe_m >= 25.0 and not tp1_hit_m:
                    pos_m["sl"] = entree_m + (0.001 * entree_m) if is_long_m else entree_m - (0.001 * entree_m)
                if roe_m >= 50.0 and not tp1_hit_m:
                    pos_m["sl"] = entree_m + (0.003 * entree_m) if is_long_m else entree_m - (0.003 * entree_m)

                # Time-stop (1h30 max)
                if duree_m > 5400 and abs(roe_m) < 15.0:
                    compte["solde"] += pnl_flottant_m
                    mettre_a_jour_ia_collective(trader_courant, paire_m, "Squeeze Master", pnl_flottant_m >= 0, pnl_flottant_m)
                    compte["historique"].insert(0, {
                        "strat": "Crypto Master", "paire": paire_m, "sens": sens_m,
                        "pnl": round(pnl_flottant_m, 2), "win": pnl_flottant_m >= 0, "date": f"{heure_fr_trade} (Time-Stop)"
                    })
                ...
                if is_long_m:
                    if not tp1_hit_m and p_direct >= tp1_m:
                        pos_m["tp1_hit"] = True
                        pnl_60 = ((tp1_m - entree_m) / entree_m) * (notionnel_m * 0.60)
                        compte["solde"] += pnl_60
                        pos_m["sl"] = entree_m + (pos_m.get("dist", 1.0) * 0.35)
                    elif tp1_hit_m and p_direct >= tp2_m:
                        pnl_40 = ((tp2_m - entree_m) / entree_m) * (notionnel_m * 0.40)
                        pnl_tot = (((tp1_m - entree_m) / entree_m) * (notionnel_m * 0.60) + pnl_40)
                        compte["solde"] += pnl_40
                        mettre_a_jour_ia_collective(trader_courant, paire_m, "Squeeze Breakout", True, pnl_tot)
                        compte["historique"].insert(0, {
                            "strategie": f"👑 Crypto Master 1M XP", "paire": paire_m, "sens": sens_m,
                            "pnl": round(pnl_tot, 2), "win": True, "date": heure_fr_trade
                        })
                        del compte["positions"][pos_k]
                    elif p_direct <= sl_m:
                        pnl_tot = (((tp1_m - entree_m) / entree_m) * (notionnel_m * 0.60) if tp1_hit_m else ((sl_m - entree_m) / entree_m) * notionnel_m)
                        compte["solde"] += (((sl_m - entree_m) / entree_m) * (notionnel_m * 0.40) if tp1_hit_m else pnl_tot)
                        mettre_a_jour_ia_collective(trader_courant, paire_m, "Squeeze Breakout", tp1_hit_m, pnl_tot)
                        compte["historique"].insert(0, {
                            "strategie": f"👑 Crypto Master 1M XP", "paire": paire_m, "sens": sens_m,
                            "pnl": round(pnl_tot, 2), "win": tp1_hit_m, "date": heure_fr_trade
                        })
                        del compte["positions"][pos_k]
                else:
                    if not tp1_hit_m and p_direct <= tp1_m:
                        pos_m["tp1_hit"] = True
                        pnl_60 = ((entree_m - tp1_m) / entree_m) * (notionnel_m * 0.60)
                        compte["solde"] += pnl_60
                        pos_m["sl"] = entree_m - (pos_m.get("dist", 1.0) * 0.35)
                    elif tp1_hit_m and p_direct <= tp2_m:
                        pnl_40 = ((entree_m - tp2_m) / entree_m) * (notionnel_m * 0.40)
                        pnl_tot = (((entree_m - tp1_m) / entree_m) * (notionnel_m * 0.60) + pnl_40)
                        compte["solde"] += pnl_40
                        mettre_a_jour_ia_collective(trader_courant, paire_m, "Squeeze Breakout", True, pnl_tot)
                        compte["historique"].insert(0, {
                            "strategie": f"👑 Crypto Master 1M XP", "paire": paire_m, "sens": sens_m,
                        ...
                        pnl_tot = (((entree_m - tp1_m) / entree_m) * (notionnel_m * 0.60) if tp1_hit_m else ((entree_m - sl_m) / entree_m) * notionnel_m)
                        compte["solde"] += (((entree_m - sl_m) / entree_m) * (notionnel_m * 0.40) if tp1_hit_m else pnl_tot)
                        mettre_a_jour_ia_collective(trader_courant, paire_m, "Squeeze Breakout", tp1_hit_m, pnl_tot)
                        compte["historique"].insert(0, {
                            "strategie": f"👑 Crypto Master 1M XP", "paire": paire_m, "sens": sens_m,
                            "pnl": round(pnl_tot, 2), "win": tp1_hit_m, "date": heure_fr_trade
        ...
        if compte.get("auto_actif", False):
            for p_nom in LISTE_PROFILS:
                param_strat = PARAMETRES_STRATS.get(p_nom, {"levier": 50, "marge": 35.0, "max_duree_sec": 1080})
                ...
                for d in donnees_tous_profils.get(p_nom, []):
                    ...
                    p_live = prix_mexc_direct.get(paire_reelle, d.get("Prix", 0))
                    ...
                    if cle_pos in compte.get("positions", {}):
                        ...
                        pnl_flottant = (((p_live - p_entree) / p_entree) * notionnel if is_long else ((p_entree - p_live) / p_entree) * notionnel)
                        roe_trade = (pnl_flottant / marge_p) * 100

                        if roe_trade >= 25.0 and not tp1_hit:
                            pos["sl"] = p_entree + (0.001 * p_entree) if is_long else p_entree - (0.001 * p_entree)
                        if roe_trade >= 50.0 and not tp1_hit:
                            pos["sl"] = p_entree + (0.003 * p_entree) if is_long else p_entree - (0.003 * p_entree)

                        if duree_trade > max_duree:
                            compte["solde"] += pnl_flottant
                            mettre_a_jour_ia_collective(trader_courant, paire_reelle, motif_fam, pnl_flottant >= 0, pnl_flottant)
                            compte["historique"].insert(0, {
                                "strategie": p_nom, "paire": paire_reelle, "sens": sens,
                                "pnl": round(pnl_flottant, 2), "win": pnl_flottant >= 0, "date": f"{heure_fr_trade} (Time-Stop)"
                            })
                        ...
                        if not is_long:
                            if not tp1_hit and p_live <= tp1:
                                pos["tp1_hit"] = True
                                pnl_50 = ((p_entree - tp1) / p_entree) * (notionnel * 0.5)
                                compte["solde"] += pnl_50
                                pos["sl"] = p_entree
                            elif tp1_hit and p_live <= tp2:
                                pnl_runner = ((p_entree - tp2) / p_entree) * (notionnel * 0.5)
                                pnl_tot = ((p_entree - tp1) / p_entree) * (notionnel * 0.5) + pnl_runner
                                compte["solde"] += pnl_runner
                                mettre_a_jour_ia_collective(trader_courant, paire_reelle, motif_fam, True, pnl_tot)
                                compte["historique"].insert(0, {
                                    "strategie": p_nom, "paire": paire_reelle, "sens": sens,
                                    "pnl": round(pnl_tot, 2), "win": True, "date": heure_fr_trade
                                })
                                ...
                                pnl = ((p_entree - sl) / p_entree) * notionnel if not tp1_hit else ((p_entree - tp1) / p_entree) * (notionnel * 0.5)
                                if not tp1_hit:
                                    compte["solde"] += pnl
                                mettre_a_jour_ia_collective(trader_courant, paire_reelle, motif_fam, tp1_hit, pnl)
                                compte["historique"].insert(0, {
                                    "strategie": p_nom, "paire": paire_reelle, "sens": sens,
                                    "pnl": round(pnl, 2), "win": tp1_hit, "date": heure_fr_trade
                                })
                                del compte["positions"][cle_pos]
                        else:
                            if not tp1_hit and p_live >= tp1:
                                pos["tp1_hit"] = True
                                pnl_50 = ((tp1 - p_entree) / p_entree) * (notionnel * 0.5)
                                compte["solde"] += pnl_50
                                pos["sl"] = p_entree
                            elif tp1_hit and p_live >= tp2:
                                pnl_runner = ((tp2 - p_entree) / p_entree) * (notionnel * 0.5)
                                pnl_tot = ((tp1 - p_entree) / p_entree) * (notionnel * 0.5) + pnl_runner
                                compte["solde"] += pnl_runner
                                mettre_a_jour_ia_collective(trader_courant, paire_reelle, motif_fam, True, pnl_tot)
                                compte["historique"].insert(0, {
                                    "strategie": p_nom, "paire": paire_reelle, "sens": sens,
                                ...
                                pnl = ((sl - p_entree) / p_entree) * notionnel if not tp1_hit else ((tp1 - p_entree) / p_entree) * (notionnel * 0.5)
                                if not tp1_hit:
                                    compte["solde"] += pnl
                                mettre_a_jour_ia_collective(trader_courant, paire_reelle, motif_fam, tp1_hit, pnl)
                                compte["historique"].insert(0, {
                    ...
                    elif len(compte["positions"]) < 4 and d.get("Signal_Detecte"):
                        compte["positions"][cle_pos] = {
                            "strategie": p_nom, "paire": paire_reelle, "sens": d["Signal_Detecte"],
                            "motif": d["Motif"], "entree": d["Opt_Price"], "sl": d["SL"],
                            "tp1": d["TP1"], "tp2": d["TP2"], "marge": marge_strat, "levier": levier_strat,
                            "tp1_hit": False, "date_open": heure_fr_trade, "open_timestamp": ts_maintenant
                        }

        # Sauvegarder automatiquement (en Cloud Upstash & local)
        sauvegarder_tous_les_comptes(compte_actuel)
    ...
    tab_auto, tab_radar, tab_master, tab_ia, tab_classement, tab_calc = st.tabs(
        [t("tab_auto", user=trader_courant), t("tab_radar", profile=profil_cle), t("tab_master"), t("tab_ia"), t("tab_rank"), t("tab_calc")]
    )
    ...
    with tab_master:
        ...
        with col_sm1:
            st.markdown(f"#### {t('master_header')} <span class='user-badge'>👤 {trader_courant}</span>", unsafe_allow_html=True)
        with col_sm2:
            mode_auto_master = st.toggle(t("toggle_master_auto"), value=c_fresh.get("master_auto", False), key="toggle_master_auto_switch_v22")
        ...
        with sel_c1:
            choix_crypto_master = st.selectbox(
                "Sélectionner la crypto Master :", bases_actives,
                index=bases_actives.index(st.session_state.selected_master_crypto) if st.session_state.selected_master_crypto in bases_actives else 0
            )
        ...
        if mode_auto_master:
            st.markdown(f"""<div style="background-color:rgba(20,241,149,0.12); padding:8px; border-radius:6px; border:1px solid #14F195; font-size:13px; color:#14F195; font-weight:bold; margin-bottom:8px;">
                {t('master_auto_on')} (Marge : 100 USDT | Levier x25)
            </div>""", unsafe_allow_html=True)
        else:
            st.markdown(f"""<div style="background-color:#0D1117; padding:8px; border-radius:6px; border:1px dashed #30363D; font-size:13px; color:#8B949E; margin-bottom:8px;">
                {t('master_auto_off')}
            </div>""", unsafe_allow_html=True)
        ...
        if master_data_live:
            ...
            regime_titre = t("regime_comp") if sq_on else t("regime_exp")
            ...
            if sq_on:
                ...
                with col_g1:
                    st.markdown(f"<b>{t('orders_buy')}</b>", unsafe_allow_html=True)
                    for lvl in master_data_live["grid_levels"]:
                        st.markdown(f"<span class='grid-badge-buy'>Achat L{lvl['lvl']} : {lvl['buy']} $</span> (-{lvl['lvl']*0.35:.2f}%)", unsafe_allow_html=True)
                with col_g2:
                    st.markdown(f"<b>{t('orders_sell')}</b>", unsafe_allow_html=True)
                    for lvl in master_data_live["grid_levels"]:
                        st.markdown(f"<span class='grid-badge-sell'>Vente L{lvl['lvl']} : {lvl['sell']} $</span> (+{lvl['lvl']*0.35:.2f}%)", unsafe_allow_html=True)
            ...
            if bo:
                ...
                if not mode_auto_master:
                    if st.button(t("take_breakout_btn", user=trader_courant), key=f"btn_manual_take_master_{choix_crypto_master}"):
                        def ajouter_pos_manuel(c):
                            cle_pos_m = f"Master_{choix_crypto_master}/USDT"
                            c["positions"][cle_pos_m] = {
                                "strategie": f"👑 {choix_crypto_master} Master 1M XP",
                        ...
                        mettre_a_jour_un_compte(trader_courant, ajouter_pos_manuel)
                        st.success(f"✅ Position {choix_crypto_master} Master ({trader_courant}) OK !")
                        st.rerun()
    ...
    with tab_auto:
        ...
        with col_t1:
            st.markdown(f"💰 <b>{t('balance_realized')}</b> `{c_fresh['solde']:.2f} USDT` | **PnL :** <span style='color:{'#00E676' if pnl_auto >= 0 else '#FF5252'}; font-weight:bold;'>{pnl_auto:+.2f} USDT</span>", unsafe_allow_html=True)
        with col_t2:
            nouvel_etat = st.toggle(t("toggle_auto_radar"), value=c_fresh.get("auto_actif", False), key="toggle_auto_live_radar_v22")
        ...
        if c_fresh.get("positions"):
            ...
            for cle, pos in list(c_fresh["positions"].items()):
                ...
                paire_nom = pos.get("paire", cle.split("_")[1] if "_" in cle else "SOL/USDT")
                ...
                tp1_statut = t("breakeven") if pos.get("tp1_hit", False) else t("waiting")

                p_actuel = prix_mexc_direct.get(paire_nom, entree_val)
                if entree_val > 0:
                    pnl_flottant = (((p_actuel - entree_val) / entree_val) * notionnel_nom if "LONG" in sens_nom else ((entree_val - p_actuel) / entree_val) * notionnel_nom)
                ...
                chrono_str = f"{hours}h {mins}m" if hours > 0 else (f"{mins}m {secs}s" if mins > 0 else f"{secs}s")
                ...
                with col_p1:
                    ...
                    """, unsafe_allow_html=True)
                with col_p2:
                    if st.button(t("cut_btn"), key=f"btn_close_pos_{cle}_v22", help=f"Close {paire_nom} at market price"):
                        def couper_pos(c):
                            if cle in c.get("positions", {}):
                                c["solde"] += pnl_flottant
                                c["historique"].insert(0, {
                                    "strategie": strat_nom, "paire": paire_nom, "sens": sens_nom,
                                    "pnl": round(pnl_flottant, 2), "win": pnl_flottant >= 0, "date": obtenir_date_heure_paris("%H:%M:%S")
                                })
                                del c["positions"][cle]
        ...
        if c_fresh.get("historique"):
            ...
            st.dataframe(pd.DataFrame(c_fresh["historique"][:6]), hide_index=True)

        if st.button(t("reset_btn"), key="btn_reset_v22"):
            def reset_c(c):
                c["solde"] = 1000.0
    ...
    with tab_radar:
        ...
        if memoire_active:
            for p, info in list(memoire_active.items()):
                temps_restant = int(durees_profils.get(profil_cle, 120) - (maintenant_ts - info["timestamp"]))
                if temps_restant > 0:
                    classe = "alert-card-long" if "LONG" in info["signal"] else "alert-card-short"
                    ...
                    ecart_pct = (p_reel - p_entree) / p_entree if is_long else (p_entree - p_reel) / p_entree
                    est_perime = ecart_pct > 0.0025

                    badge_statut = f'<span class="status-expired">{t("status_expired")}</span>' if est_perime else f'<span class="status-valid">{t("status_valid")}</span>'
                    ...
                    if not est_perime:
                        if st.button(t("take_signal_btn", sens=info['signal'], pair=p, user=trader_courant), key=f"btn_radar_take_{p}_v22"):
                            def prendre_pos_radar(c):
                                cle = f"{profil_cle}_{p}"
                                p_strat = PARAMETRES_STRATS.get(profil_cle, {"levier": 50, "marge": 35.0})
                                c["positions"][cle] = {
                                    "strategie": profil_cle, "paire": p,
                                    "sens": "LONG" if "LONG" in info["signal"] else "SHORT",
                                    "motif": info["motif"], "entree": info["prix_entree"], "sl": info["sl"],
                                    "tp1": info["tp1"], "tp2": info["tp2"], "marge": p_strat["marge"],
                                    "levier": p_strat["levier"], "tp1_hit": False,
                                    "date_open": obtenir_date_heure_paris("%H:%M:%S"), "open_timestamp": time.time()
                                }
                            mettre_a_jour_un_compte(trader_courant, prendre_pos_radar)
                            st.success(f"✅ Position {p} ({trader_courant}) OK !")
                            st.rerun()

        donnees_dict = {d["Paire"]: d for d in donnees_tous_profils.get(profil_cle, [])}
        lignes_tableau = []
        ...
        for nom_court in bases_actives:
            ...
            statut = memoire_active[paire_nom]["signal"] if paire_nom in memoire_active else t("veille")
            prix_reel_mexc = prix_mexc_direct.get(paire_nom, d.get("Prix", "N/A"))
            mtf = obtenir_stats_mtf_radar(nom_court)

            lignes_tableau.append({
                t("pair_col"): paire_nom, t("price_col"): formater_prix(prix_reel_mexc), t("status_col"): statut,
                "Range 15m": mtf["range_15m"], "RSI 15m": mtf["rsi_15m"], "Range 30m": mtf["range_30m"],
                "RSI 30m": mtf["rsi_30m"], "Range 1h": mtf["range_1h"], "RSI 1h": mtf["rsi_1h"],
                "Range 4h": mtf["range_4h"], "RSI 4h": mtf["rsi_4h"], "Range 1j": mtf["range_1d"], "RSI 1j": mtf["rsi_1d"]
            })

        st.dataframe(pd.DataFrame(lignes_tableau), hide_index=True, use_container_width=True)
    ...
    # ======================================================
    # 🏆 CLASSEMENT LIVE : THOMAS, ALEX & YEEPSE !
    # ======================================================
    with tab_classement:
        liste_classement = []
        ...
        for nom, c in comptes_live.items():
            ...
            liste_classement.append({
                "Trader": f"👤 {nom}", "Solde / Balance": f"{c['solde']:.1f} $",
                "PnL": f"{pnl:+.1f} $", "WR": f"{wr:.0f}%", "Trades": trades_nb
            })
        st.dataframe(pd.DataFrame(liste_classement).sort_values(by="PnL", ascending=False), hide_index=True, use_container_width=True)

    with tab_calc:
        c1, c2 = st.columns(2)
        paire_sel = c1.selectbox(t("pair_col"), [f"{p}/USDT" for p in bases_actives])
        sens_sel = c2.radio("Sens / Direction", ["LONG 🟢", "SHORT 🔴"], horizontal=True)
        is_long = "LONG" in sens_sel

        col_e, col_sl = st.columns(2)
        p_entree = col_e.number_input(t("calc_entry"), value=106.20, step=0.01, format="%.5f")
        p_sl = col_sl.number_input(t("calc_sl"), value=105.90, step=0.01, format="%.5f")
        ...
        if (is_long and p_sl < p_entree) or ((not is_long) and p_sl > p_entree):
            ...
            p_liq = p_entree * (1 - dist_liq_pct) if is_long else p_entree * (1 + dist_liq_pct)
            tp1 = p_entree + (1.8 * distance) if is_long else p_entree - (1.8 * distance)
            tp2 = p_entree + (3.8 * distance) if is_long else p_entree - (3.8 * distance)
